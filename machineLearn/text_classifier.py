...
import numpy as np
import nltk
# Downloads of auxiliary text manipulation libraries
...
if __name__ == '__main__':
    ...
    # The next step is broken the texts in spaces, ',', '?' and others characters
    broken_texts = [nltk.tokenize.word_tokenize(text) for text in texts ]
    ...
    for word_list in broken_texts:
        # Rejected stopwords and words with len < 2
        valid_words = [stemmer.stem(word) for word in word_list if word not in stopwords and len(word) > 2]
